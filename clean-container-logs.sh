...
if [[ $CLEAN_ALL -eq 0 ]] && [[ ${#CONTAINERS[@]} -eq 0 ]]; then
    echo "请指定容器名或使用 --all" >&2; usage; exit 1
fi

[[ $CLEAN_ALL -eq 1 ]] && CONTAINERS=($(docker ps -aq))

CLEANED=0
for name in "${CONTAINERS[@]}"; do
    CID=$(docker inspect -f '{{.Id}}' "$name" 2>/dev/null || true)
    [[ -z "$CID" ]] && echo "容器不存在: $name" && continue

    LOG_FILE="${LOG_DIR}/${CID}/${CID}-json.log"
    [[ ! -f "$LOG_FILE" ]] && echo "无日志文件: $name" && continue
...
echo
[[ "$DRY_RUN" == 0 ]] && echo "清理了 $CLEANED 个容器日志。"
