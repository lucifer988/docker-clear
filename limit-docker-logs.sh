...
human_size() {
    ...
    if command -v bc >/dev/null 2>&1; then
        echo "$mb" | bc -l | awk '{if($1>1024)printf "%.2f GB\n",$1/1024;else printf "%.2f MB\n",$1}' && return
    fi
    # 无 bc 时的简单 fallback
...
# 检测当前日志驱动，不强制覆盖用户已有的非 json-file 配置
...
# 修复 heredoc 变量安全：用 base64 过渡
SAFE_JSON=$(echo "$NEW_JSON" | base64 -w0)

if [[ "$DRY_RUN" == 0 ]]; then
    FINAL_JSON=$(echo "$SAFE_JSON" | base64 -d)
    echo "$FINAL_JSON" > "$DAEMON_JSON"
